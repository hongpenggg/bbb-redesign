  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="py-16 px-6 bg-surface-container border-y-2 border-ink-black/10">
        <div className="max-w-container mx-auto">
          <div className="max-w-3xl mx-auto">
            <h2 className="font-display text-[28px] text-ink-black mb-6 text-center">
              About Bit by Bit Coding
            </h2>
            <div className="bg-surface p-8 md:p-10 wiggly-border hard-shadow">
      ...
      <section className="py-16 px-6">
        <div className="max-w-container mx-auto">
          <h2 className="font-display text-[28px] text-ink-black mb-8 text-center">
            Key Facts
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
            {keyFacts.map((fact, i) => (
              <div
                key={i}
                className={`bg-surface p-5 wiggly-border hard-shadow text-center hover-wiggle transform ${
                  i % 3 === 0 ? '-rotate-1' : i % 3 === 2 ? 'rotate-1' : ''
                }`}
              >
                <div className="font-display text-[24px] font-bold text-primary mb-1">
                  {fact.value}
                </div>
                <div className="font-label font-bold text-[12px] text-on-surface-variant uppercase tracking-wider">
                  {fact.label}
                </div>
              </div>
            ))}
        ...
        <div className="max-w-container mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {assets.map((asset, i) => (
              <div
                key={i}
                className={`bg-surface p-6 wiggly-border hard-shadow hover-wiggle flex flex-col items-center text-center gap-4 transform ${
                  i % 3 === 0 ? '-rotate-1' : i % 3 === 2 ? 'rotate-1' : ''
                }`}
              >
                <div className="w-14 h-14 bg-primary-fixed wiggly-border-thin flex items-center justify-center">
                  <asset.icon className="w-7 h-7 text-primary" />
                </div>
                <div>
                  <h3 className="font-display text-[16px] font-bold text-ink-black mb-1">
                    {asset.title}
                  </h3>
                  <p className="text-on-surface-variant text-[12px] leading-relaxed mb-3">
                    {asset.description}
                  </p>
      ...
      <section className="py-16 px-6">
        <div className="max-w-container mx-auto">
          <div className="max-w-2xl mx-auto">
            <div className="bg-surface p-8 md:p-10 wiggly-border hard-shadow-lg text-center">
              <h2 className="font-display text-[24px] font-bold text-ink-black mb-4">
                Press Contact
              </h2>
              ...
              <div className="flex flex-wrap justify-center gap-4">
                <a
                  ...
                  className="inline-flex items-center gap-2 bg-primary text-on-primary font-label font-bold text-[14px] px-6 py-3 wiggly-border hard-shadow hover-wiggle"
                >
                  Email Us
                </a>
                <a
                  ...
                  className="inline-flex items-center gap-2 border-2 border-ink-black text-ink-black font-label font-bold text-[14px] px-6 py-3 wiggly-border hover:bg-surface-container-high transition-colors"
                >
      ...
      <section className="pb-16 px-6 text-center">
        <Link
          to="/"
          className="inline-flex items-center gap-2 text-primary font-label font-bold text-[14px] hover:underline"
        >
          ← Back to Home
        </Link>
      </section>
