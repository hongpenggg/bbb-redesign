        ...
        <div className="max-w-container mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 mb-10 md:mb-16">
            {contactInfo.map((info, i) => (
              <div
                key={i}
                className={`bg-surface p-5 md:p-6 wiggly-border hard-shadow hover-wiggle text-center transform ${
                  i % 3 === 0 ? '-rotate-1' : i % 3 === 2 ? 'rotate-1' : ''
                }`}
              >
                <div className={`w-14 h-14 md:w-16 md:h-16 ${info.color} wiggly-border-thin flex items-center justify-center mx-auto mb-4`}>
                  <info.icon className="w-7 h-7 md:w-8 md:h-8 text-on-primary" />
                </div>
                <h3 className="font-display text-[16px] md:text-[18px] font-bold mb-2 text-ink-black">{info.title}</h3>
                <p className="text-[12px] md:text-[13px] text-on-surface-variant mb-2">{info.description}</p>
                <p className="text-[12px] md:text-[13px] font-label font-bold text-primary">{info.detail}</p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-surface wiggly-border hard-shadow-lg overflow-hidden">
            <div className="grid grid-cols-1 lg:grid-cols-12">
              {/* Form Side */}
              <div className="lg:col-span-7 p-5 md:p-8 lg:p-12 bg-surface">
                <div className="mb-4 md:mb-6">
                  <h2 className="text-[24px] sm:text-[28px] md:text-[32px] font-display font-bold mb-2 md:mb-3 text-ink-black leading-tight">
                    Drop Us a Message
                  </h2>
                ...
                <div className="flex-grow">
                  <iframe
                    ...
                    className="w-full h-[700px] md:h-[842px]"
                  />
                </div>
              </div>
              ...
              <div className="lg:col-span-5 bg-gradient-to-br from-primary to-primary-container p-6 md:p-10 lg:p-12 flex flex-col justify-between relative overflow-hidden text-white">
                ...
                <div className="relative z-10">
                  <h3 className="text-[20px] md:text-[24px] lg:text-[28px] font-display font-bold mb-6">
                    Why Reach Out?
                  </h3>
                  <div className="space-y-5 md:space-y-6">
                    {[
                      ...
                      <div key={i} className="flex gap-3 md:gap-4">
                        ...
                        <div>
                          <h4 className="font-bold text-white text-[14px] md:text-[16px] mb-1">{item.heading}</h4>
                          <p className="text-[12px] md:text-[13px] text-white/80 leading-relaxed">{item.body}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="relative z-10 mt-6 md:mt-8">
                  <div className="bg-white/10 border border-white/30 p-4 md:p-6 wiggly-border-thin">
                    <p className="text-[12px] md:text-[13px] text-white/90 leading-relaxed italic">
                      &ldquo;Bit by Bit isn&apos;t just about coding &mdash; it&apos;s about building a community of young innovators who will shape Singapore&apos;s future.&rdquo;
                    </p>
                    <p className="text-[11px] md:text-[12px] text-white/70 mt-3 font-bold">&mdash; The Bit by Bit Team</p>
                  </div>
                </div>
              </div>
            </div>
